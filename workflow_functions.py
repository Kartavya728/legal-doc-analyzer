...
def personal_workflow(chunks, category, doc_text):
    ...
    # Save to DataFrame
    df = pd.DataFrame({"Clause": unique_clauses, "Level1": category})
    doc_type = personal_.classify_personal_document(doc_text)
...
def contarct_workflow(chunks):
    ...
    # Save to DataFrame
    df = pd.DataFrame({"Clause": unique_clauses, "Level1": "Contracts & Agreements"})
    ...
    return contract_.makenice(explained_contract_clauses), #JSON KKAHAN HAI ISKI DHOONDLIO
...
def govt_workflow(chunks):
    ...
    government_attributes = [
        {"clause": c}
        for c in government_unique_clauses
    ]
    ...
    return  govt_.summarize_government_from_json(merged_government, client), #JSON KAHAN HAI IS CATEGORY KI DEKHNI HAI
